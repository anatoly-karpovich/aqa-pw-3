...
test.describe(`[UI] [Home] Metrics`, async function () {
  test(
    "Should see Orders This Year metric with 10",
    { tag: ["@regression"] },
  ...
  test(
    "Should see Total Revenue metric with 100000000",
    { tag: ["@regression"] },
    async function ({ signInPageService, mock, homePageService }) {
      const mockData = structuredClone(homePageMock);
  ...
  test(
    "Should see Avg Order Value metric with 10000",
    { tag: ["@regression"] },
    async function ({ signInPageService, mock, homePageService }) {
      const mockData = structuredClone(homePageMock);
  ...
  test(
    "Should see New Customers metric with 100",
    { tag: ["@regression"] },
    async function ({ signInPageService, mock, homePageService }) {
      const mockData = structuredClone(homePageMock);
  ...
  test(
    "Should see Canceled Orders metric with 5",
    { tag: ["@regression"] },
    async function ({ signInPageService, mock, homePageService }) {
      const mockData = structuredClone(homePageMock);
